...
#[tokio::test]
#[ignore = "requires SQL_CONNECTOR_MILVUS_E2E_* environment variables"]
#[allow(clippy::too_many_lines)]
async fn milvus_vector_crud_is_policy_controlled_over_mcp_worker() {
    let endpoint = env::var("SQL_CONNECTOR_MILVUS_E2E_ENDPOINT").unwrap();
    let token = env::var("SQL_CONNECTOR_MILVUS_E2E_TOKEN").unwrap();
    let target = env::var("SQL_CONNECTOR_MILVUS_E2E_COLLECTION").unwrap();
    let executable = env::var_os("SQL_CONNECTOR_MILVUS_E2E_WORKER").unwrap();

    let connection_id = ConnectionId::new();
    let profile = ConnectionProfile {
        id: connection_id,
        display_name: "Milvus live".into(),
        product: Product::Milvus,
        api_mode: "milvus_rest_v2".into(),
        ...
        options: BTreeMap::from([
            ("vector_field".into(), json!("vector")),
            ("primary_key_field".into(), json!("id")),
        ]),
    };
    ...
    let manifest = worker
        ...
        .iter()
        .find(|manifest| {
            manifest.product == Product::Milvus && manifest.api_mode == "milvus_rest_v2"
        })
        .unwrap()
        .clone();
    ...
    assert_eq!(capabilities["id"], "milvus-rest-v2");
    ...
    assert_eq!(connection_info["product_name"], "Milvus");
    assert_eq!(connection_info["api_mode"], "milvus_rest_v2");
    ...
    assert!(
        described["fields"].as_array().unwrap().iter().any(|field| {
            field["name"]["value"] == "id" && field["primaryKey"]["value"] == true
        })
    );
    assert!(described["fields"].as_array().unwrap().iter().any(|field| {
        field["name"]["value"] == "vector" && field["type"]["value"] == "FloatVector"
    }));

    let upsert_arguments = json!({
        "connection_id": connection_id,
        "request_id": "milvus-upsert-1",
        "request": {
            "target": target,
            "points": [
                {
        ...
        let result = success(
            client
                .call_tool(tool_params(
                    ...
                    &json!({
                        "connection_id": connection_id,
                        "request_id": format!("milvus-search-{attempt}"),
                        "request": {
                            "target": target,
                            "vector": [1.0, 0.01, 0.0],
                            "top_k": 2,
    ...
    let searched = searched.expect("Milvus bounded consistency makes the upsert searchable");
    assert_eq!(searched["records"].as_array().unwrap().len(), 1);
    assert_eq!(searched["records"][0]["id"]["value"], 910001);
    ...
    let delete_arguments = json!({
        ...
        "request": {
            ...
            "filter": {
                ...
                "field": "id",
                "values": [
                    {"type": "int64", "value": 910001},
                    {"type": "int64", "value": 910002}
                ]
            },
            "max_affected": 2,
            "idempotency_key": null
    ...
    let mut delete_is_visible = false;
    for attempt in 1..=30 {
        let mut fetch_after_delete = fetch_arguments.clone();
        fetch_after_delete["request_id"] = json!(format!("milvus-fetch-after-delete-{attempt}"));
        let fetched = success(
            client
                .call_tool(tool_params("vector_fetch", &fetch_after_delete))
                .await
                .unwrap(),
        );
        if fetched["records"].as_array().unwrap().is_empty() {
    ...
    assert!(
        delete_is_visible,
        "Milvus delete becomes visible to queries"
    );
