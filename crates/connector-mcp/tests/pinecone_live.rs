...
#[tokio::test]
#[ignore = "requires SQL_CONNECTOR_PINECONE_E2E_* environment variables"]
#[allow(clippy::too_many_lines)]
async fn pinecone_vector_crud_is_policy_controlled_over_mcp_worker() {
    let endpoint = env::var("SQL_CONNECTOR_PINECONE_E2E_ENDPOINT").unwrap();
    let api_key = env::var("SQL_CONNECTOR_PINECONE_E2E_API_KEY").unwrap();
    let target = env::var("SQL_CONNECTOR_PINECONE_E2E_INDEX").unwrap();
    let executable = env::var_os("SQL_CONNECTOR_PINECONE_E2E_WORKER").unwrap();

    let connection_id = ConnectionId::new();
    let profile = ConnectionProfile {
        id: connection_id,
        display_name: "Pinecone live".into(),
        product: Product::Pinecone,
        api_mode: "pinecone_2025_10".into(),
        ...
        options: BTreeMap::from([("namespace".into(), json!("sql-connector-live"))]),
    };
    ...
    let manifest = worker
        ...
        .iter()
        .find(|manifest| {
            manifest.product == Product::Pinecone && manifest.api_mode == "pinecone_2025_10"
        })
        .unwrap()
        .clone();
    ...
    assert_eq!(capabilities["id"], "pinecone-2025-10");
    assert_eq!(capabilities["resource_target"]["kind"], "vector_index");
    ...
    assert_eq!(connection_info["product_name"], "Pinecone");
    assert_eq!(connection_info["api_mode"], "pinecone_2025_10");
    ...
    assert_eq!(described["metadata"]["dimension"]["value"], 3);
    assert_eq!(described["metadata"]["metric"]["value"], "cosine");

    let upsert_arguments = json!({
        "connection_id": connection_id,
        "request_id": "pinecone-upsert-1",
        "request": {
            "target": target,
            "points": [
                {
    ...
    let fetch_arguments = json!({
        ...
        "request": {
            ...
            "filter": {
                "op": "eq",
                "field": "id",
                "value": {"type": "string", "value": "sql-live-1"}
            },
    ...
    let delete_arguments = json!({
        ...
        "request": {
            ...
            "filter": {
                ...
                "field": "id",
                "values": [
                    {"type": "string", "value": "sql-live-1"},
                    {"type": "string", "value": "sql-live-2"}
                ]
            },
            "max_affected": 2,
            "idempotency_key": null
    ...
    let mut fetch_after_delete = fetch_arguments;
    fetch_after_delete["request_id"] = json!("pinecone-fetch-after-delete");
    let fetched = success(
        client
