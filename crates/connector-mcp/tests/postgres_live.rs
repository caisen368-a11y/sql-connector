...
#[tokio::test]
#[ignore = "requires SQL_CONNECTOR_POSTGRES_E2E_* environment variables"]
#[allow(clippy::too_many_lines)]
async fn postgres_connection_is_identified_and_crud_is_policy_controlled_over_mcp() {
    let endpoint = env::var("SQL_CONNECTOR_POSTGRES_E2E_ENDPOINT").unwrap();
    let database = env::var("SQL_CONNECTOR_POSTGRES_E2E_DATABASE").unwrap();
    let username = env::var("SQL_CONNECTOR_POSTGRES_E2E_USERNAME").unwrap();
    let password = env::var("SQL_CONNECTOR_POSTGRES_E2E_PASSWORD").unwrap();
    let ca_certificate =
        fs::read_to_string(env::var("SQL_CONNECTOR_POSTGRES_E2E_CA_CERTIFICATE_FILE").unwrap())
            .unwrap();
    let tls_server_name = env::var("SQL_CONNECTOR_POSTGRES_E2E_TLS_SERVER_NAME").unwrap();
    let expected_version_prefix =
        env::var("SQL_CONNECTOR_POSTGRES_E2E_EXPECTED_VERSION_PREFIX").unwrap();
    let executable = env::var_os("SQL_CONNECTOR_POSTGRES_E2E_WORKER").unwrap();

    let connection_id = ConnectionId::new();
    let target = "public.items".to_owned();
    let owners_target = "public.owners".to_owned();
    let profile = ConnectionProfile {
        id: connection_id,
        display_name: "PostgreSQL live".into(),
    ...
    let manifest = worker
        ...
        .iter()
        .find(|manifest| {
            manifest.product == Product::PostgreSql && manifest.api_mode == "postgresql"
        })
        .unwrap()
        .clone();
    ...
    wrong_host_profile
        ...
        .unwrap();
    wrong_host_profile.tls.server_name = Some("127.0.0.1".into());
    ...
    assert_eq!(listed[0]["product"], "postgresql");
    ...
    assert_eq!(capabilities["id"], "postgresql-pgwire");
    ...
    assert_eq!(connection_info["product_name"], "PostgreSQL");
    assert!(
        ...
        scoped_client
            .call_tool(tool_params(
                ...
                &json!({
                    ...
                    "request_id": "postgres-policy-query-1",
                    "request": {
                        "language": "postgresql",
                        "statement": "SELECT o.id AS owner_id, COUNT(i.id) AS item_count FROM public.owners o JOIN public.items i ON i.owner_id = o.id GROUP BY o.id",
                        "parameters": {},
                        "positional_parameters": []
                    }
                }),
    ...
    assert_eq!(read["records"][0]["qty"]["value"], 3);
    ...
    let slow_params = tool_params(
        ...
        &json!({
            "connection_id": connection_id,
            "request_id": "postgres-sleep-1",
            "request": {
                "language": "postgresql",
                "statement": "SELECT pg_sleep(30) AS slept",
                "parameters": {},
                "positional_parameters": [],
                "max_affected": null,
                "idempotency_key": null
    ...
    let cancelled_query = timeout(Duration::from_secs(5), slow_query)
        .await
        .expect("cancelled PostgreSQL query must finish promptly")
        .unwrap();
    ...
    let recovered_native = success(
        client
            .call_tool(tool_params(
                ...
                &json!({
                    "connection_id": connection_id,
                    "request_id": "postgres-after-cancel-native-1",
                    "request": {
                        "language": "postgresql",
    ...
    recovery_read_arguments["request_id"] = json!("postgres-after-cancel-read-1");
    ...
    let interrupted_arguments = json!({
        "connection_id": connection_id,
        "request_id": "postgres-unknown-outcome-1",
        "request": {
            "language": "postgresql",
            "statement": "UPDATE \"items\" SET \"qty\" = \"qty\" + (SELECT 0 FROM pg_sleep(30)) WHERE \"id\" = $1",
            "parameters": {},
            "positional_parameters": [{"type": "int64", "value": 1}],
            "max_affected": 1,
            "idempotency_key": "postgres-unknown-outcome-key-1"
        }
    });
    ...
    let interrupted = timeout(Duration::from_secs(10), interrupted_write)
        .await
        .expect("interrupted PostgreSQL write must finish promptly")
        .unwrap();
    ...
    retry_arguments["request_id"] = json!("postgres-unknown-outcome-retry-1");
    let retry = timeout(
    ...
    .await
    .expect("PostgreSQL unknown-outcome retry must be rejected without execution")
    .unwrap();
    ...
    assert_eq!(restarted_connection["product_name"], "PostgreSQL");
    assert!(
    ...
    restarted_read_arguments["request_id"] = json!("postgres-after-restart-read-1");
