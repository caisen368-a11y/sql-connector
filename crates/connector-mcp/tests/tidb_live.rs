...
#[tokio::test]
#[ignore = "requires SQL_CONNECTOR_TIDB_E2E_* environment variables"]
#[allow(clippy::too_many_lines)]
async fn tidb_crud_and_native_write_limits_work_over_mcp_worker() {
    let endpoint = env::var("SQL_CONNECTOR_TIDB_E2E_ENDPOINT").unwrap();
    let database = env::var("SQL_CONNECTOR_TIDB_E2E_DATABASE").unwrap();
    let username = env::var("SQL_CONNECTOR_TIDB_E2E_USERNAME").unwrap();
    let password = env::var("SQL_CONNECTOR_TIDB_E2E_PASSWORD").unwrap();
    let executable = env::var_os("SQL_CONNECTOR_TIDB_E2E_WORKER").unwrap();

    let connection_id = ConnectionId::new();
    let target = format!("{database}.items");
    let profile = ConnectionProfile {
        id: connection_id,
        display_name: "TiDB live".into(),
    ...
    let manifest = worker
        ...
        .connectors
        .iter()
        .find(|manifest| manifest.product == Product::TiDb && manifest.api_mode == "mysql")
        .unwrap()
        .clone();
    ...
    assert_eq!(capabilities["id"], "tidb-mysql");
    ...
    assert_eq!(connection_info["product_name"], "TiDB");
    assert!(
    ...
    assert!(
        catalog["entities"]
            ...
            .unwrap()
            .iter()
            .any(|entity| entity["id"] == "connector_test.items")
    );
    ...
    let described = success(
        client
            .call_tool(tool_params(
                ...
                &json!({
                    "connection_id": connection_id,
                    "entity_id": "connector_test.items"
                }),
            ))
            .await
            .unwrap(),
    ...
    let native_arguments = json!({
        "connection_id": connection_id,
        "request_id": "tidb-native-update-over-limit",
        "request": {
            "language": "mysql",
