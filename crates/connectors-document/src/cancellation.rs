...
impl CancellationRegistry {
        ...
        F: Future<Output = Result<T>> + Send + 'static,
    {
        ...
        // Dropping a JoinHandle detaches it. Abort explicitly so a timed-out connector task does
        // not retain credentials or keep using a connection after it is removed from the registry.
        // A write remains UnknownOutcome because the server may already have accepted it.
        if interrupted {
            task.abort();
            let _ = task.await;
        }
...
mod tests {
    ...
    fn context(request_id: &str, duration: Duration) -> ConnectorContext {
        ConnectorContext {
    ...
    async fn cancellation_classifies_reads_and_writes_differently() {
        ...
        let read_context = context("read", Duration::from_secs(5));
        let read = tokio::spawn(async move {
            read_registry
                .run(&read_context, false, async {
        ...
        registry.cancel("read").await.unwrap();
        assert_eq!(
        ...
        let write_context = context("write", Duration::from_secs(5));
        let write = tokio::spawn(async move {
            write_registry
                .run(&write_context, true, async {
                    tokio::time::sleep(Duration::from_secs(30)).await;
                    Ok(())
                })
                .await
    ...
    #[tokio::test]
    async fn duplicate_request_does_not_replace_the_original_cancel_token() {
        let registry = CancellationRegistry::default();
        let first_registry = registry.clone();
        let first_context = context("same-id", Duration::from_secs(5));
        ...
        let duplicate_context = context("same-id", Duration::from_secs(5));
        let duplicate = registry
            .run(&duplicate_context, false, async { Ok(()) })
            .await
            .unwrap_err();
        ...
        assert_eq!(
            first.await.unwrap().unwrap_err().category,
            connector_core::ErrorCategory::Cancelled
        );
    }
}
